...
function [g, S, drag] = cal_g   

Y = read_pressure;
S = linspace(0,10,size(Y,3)); % Texture domain time

drag = zeros(size(Y,3),2); % x force, y force
g    = zeros(size(Y,3),2); % sound texture 
...
for jj = 1:2
   ...
   g(1,  jj) = (4.*drag(2    ,jj) - drag(3    ,jj) - 3.*drag(1  ,jj))/2/dt;
   g(end,jj) =-(4.*drag(end-1,jj) - drag(end-2,jj) - 3.*drag(end,jj))/2/dt;
end

end

% Construct sound texture
